...
void Testbed::imgui() {
	...
	ImGui::End();
	    ...
	    /***********************************************************************************************************************************/
    if (ImGui::Begin("Create your NeRF")) {
		if (ImGui::CollapsingHeader("Path manipulation", ImGuiTreeNodeFlags_DefaultOpen)) {
            static char vfile_path[260] = "";
            ImGui::InputText("Video file ", vfile_path, IM_ARRAYSIZE(vfile_path));
            if (ImGui::Button("Browse"))
            {
                OPENFILENAMEA ofn = {0};
                ZeroMemory(&ofn, sizeof(ofn));
                ofn.lStructSize = sizeof(ofn);
                ofn.lpstrFile = vfile_path;
                ofn.nMaxFile = sizeof(vfile_path);
                ofn.Flags = OFN_NOCHANGEDIR | OFN_ALLOWMULTISELECT | OFN_EXPLORER;
                if (GetOpenFileNameA(&ofn)){
					static std::string ini_dir;
					ini_dir = (root_dir()).str();
					if (ofn.lpstrFile[ofn.nFileOffset - 1] == '\\')
					{
						std::string command = "python "+ ini_dir +"\\scripts\\colmap2nerf.py --images "+ std::string(vfile_path) +"  --run_colmap --aabb_scale 4 --overwrite"  ;
					...
					else
					{
						try{
							...
							/*
							std::string str = std::string(vfile_path);
							char target_char = '.'; // specify the target character
							std::size_t pos = str.find_last_of(target_char); // find the last occurrence of the target character
							if (pos != std::string::npos) {
								str.erase(pos + 1); // erase everything after the target character (including the target character itself)
							}*/
							std::string command = "python "+ ini_dir +"\\scripts\\colmap2nerf.py --video_in "+ std::string(vfile_path) +" --video_fps 10 --run_colmap --aabb_scale 4 --overwrite"  ;
							system(command.c_str());
    ...
    /***********************************************************************************************************************************/
    if (ImGui::Begin("Upload your NeRF")) {
		if (ImGui::CollapsingHeader("Path manipulation", ImGuiTreeNodeFlags_DefaultOpen)) {
            static char file_path[260] = "";
            ImGui::InputText("NeRF file path", file_path, IM_ARRAYSIZE(file_path));
            if (ImGui::Button("Browse"))
            {
                OPENFILENAMEA ofn;
                ZeroMemory(&ofn, sizeof(ofn));
                ...
                if (GetOpenFileNameA(&ofn)){
					try{
                    	...
                    	set_train(true);
                   		...
                   		ImGui::OpenPopup("NeRF path load error");
                }
			...
			ImGui::SameLine();
            if (imgui_colored_button("Load NeRF", 0.f)) {
				ImGui::InputText("NeRF file path", file_path, IM_ARRAYSIZE(file_path));
                //std::string file_path_string(file_path);
                try{
                    load_training_data(file_path);
    ...
    ImGui::Begin("instant-ngp v" NGP_VERSION, 0);
    ...
    if (ImGui::CollapsingHeader("Rendering", ImGuiTreeNodeFlags_DefaultOpen)) {
        ...
        /*Advanced rendering options + Debug
